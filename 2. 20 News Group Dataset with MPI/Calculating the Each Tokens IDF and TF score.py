#EXERCISE 4: IDF x TF CALCULATING
...
def term_frequency_of_a_token(corpus,token_for_term_freq):
    tokened_corpus =  data_cleaning_one(corpus)
    number_of_the_repeated_token = tokened_corpus.count(token_for_term_freq)
    total_tokens = len(tokened_corpus)  
    return number_of_the_repeated_token/total_tokens
...
for i in range(0,size):
    if rank==i:
        ...
        for i in received: # received is a list of tokens
          #idf score token i  
            idf_score = inverse_document_freq(i, raw_corpuses)
          
          #tf score of token 
            for j in range(0,19997): # 
                corpus =raw_corpuses[j] #(jth corpus of the 19997 corpuses)
                tf = term_frequency_of_a_token(corpus,i)
                x.append("Corpus Index=:",j, " Token=:",i," IDF:=",idf_score, " Tf Score.=", tf)
                            
end = MPI.Wtime() #ending time
y = end-start #for calculationg the timing
